...
import com.example.SpringBatchDemo.job.FileDataReadWrite.dto.PlayerYears;
...
//                        items.forEach(System.out::println); // 읽어오는게 잘되는지 확인
//                    }
//                })
    ...
    /**
     * 스프링배치에서 FlatFileItemReader를 지원하기 때문에
     * 이를 활용해 file을 읽어오자.
    ...
    public FlatFileItemReader<Player> playerItemReader(){
        return new FlatFileItemReaderBuilder<Player>()
                ...
                .lineTokenizer(new DelimitedLineTokenizer()) // 콤마로 라인 구분
                .fieldSetMapper(new PlayerFieldSetMapper()) // 읽어온 데이터를 객체로 변환하도록 하는 역할
                .linesToSkip(1) // 첫번째 라인은 제목줄에 해당하므로 스킵한다.(필요한 데이터가 아님)
                .build();
    }
